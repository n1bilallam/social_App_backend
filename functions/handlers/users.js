  ...
  db.doc(`/users/${req.user.handle}`)
    ...
    .then((doc) => {
      if (doc.exists) {
        ...
        return res.statuts(404).json({ error: "User not found" });
      }
    })
    .then((data) => {
      userData.likes = [];
...
exports.uploadImage = (req, res) => {
  ...
  busboy.on("file", (fieldname, file, filename, encoding, mimetype) => {
    ...
    const imageExteension = filename.split(".")[filename.split(".").length - 1];
